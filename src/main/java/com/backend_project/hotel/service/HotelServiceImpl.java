...
public class HotelServiceImpl implements HotelService {
    ...
    public ResponseEntity<HotelModel> updateStaff(Integer id, HotelModel model) {
        return hotelRepositories.findById(id)
                .map(staff -> {
                    ...
                    staff.setDateOfBirth(model.getDateOfBirth());
                    staff.setPhonenumber(model.getPhonenumber());
                    staff.setRole(model.getRole());
                    staff.setJoiningDate(model.getJoiningDate());
                    staff.setExperience(model.getExperience());
                    staff.setQualification(model.getQualification());
    ...
    @Override
    public byte[] generateStaffPdf(Integer id) throws DocumentException {
        HotelModel staff = hotelRepositories.findById(id)
                .orElseThrow(() -> new IllegalArgumentException("Staff not found with id: " + id));
        return pdfGenerationService.generateStaffPdf(staff);
    }

    @Override
    public byte[] generateAllStaffPdf() throws DocumentException {
        List<HotelModel> staffList = hotelRepositories.findAll();
        if (CollectionUtils.isEmpty(staffList)) {
            throw new IllegalArgumentException("No staff members found");
        }
        return pdfGenerationService.generateAllStaffPdf(staffList);
    }
}